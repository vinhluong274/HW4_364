...
tags = db.Table('tags',db.Column('search_id',db.Integer, db.ForeignKey('searches.id')),db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=tags, backref=db.backref('searches', lazy='dynamic'),lazy='dynamic')
...
def get_or_create_gif(title, url):
    ...
    g = db.session.query(Gif).filter_by(title=title).first()
    if g:
        return g
    else:
...
def get_or_create_search_term(term):
    ...
    if searchTerm:
        ...
        results = get_gifs_from_giphy(term)
        for g in results:
...
def index():
    ...
    if form.validate_on_submit():
        ...
        # return redirect('/gifs_searched/'+term.term)
...
def create_collection():
    ...
    print("part 1")
    if form.validate_on_submit():
        gif_ids = [i for i in form.gif_picks.data]
        gif_objs = [get_gif_by_id(id) for id in gif_ids]
        get_or_create_collection(form.name.data, current_user, gif_objs)
        return render_template('collections.html', collections=current_user.collection)
        # return redirect(url_for('collections', collections=current_user.collection))
    else:
        print("not invoked")
